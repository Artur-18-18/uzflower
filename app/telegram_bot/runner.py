"""
Универсальный запуск Telegram-бота (polling или webhook).

Использование:
    - Локально (polling): python -m app.telegram_bot.runner --mode polling
    - Production (webhook): python -m app.telegram_bot.runner --mode webhook --webhook-url https://your-app.onrender.com/api/telegram/webhook
"""
...
async def start_webhook(webhook_url: str, port: int = 8000):
    """
    Запустить бота в режиме webhook (для production).

    Args:
        webhook_url: Полный URL webhook (например, https://your-app.onrender.com/api/telegram/webhook)
    ...
    # Регистрируем обработчик на путь /api/telegram/webhook
    handler.register(app, path="/api/telegram/webhook")
